...
import AppConstants form '../constants/app-constants';
...
const _removeItem = (item) => {
  _cartItems.splice( _cartItem.findIndex(i => i === item), 1 );
};

const _findCartItem = (item) => {
  _cartItems.find(_cartItem => cartItem.id === item.id );
};

const _increaseItem = (item) => item.qty++;

const _decreaseItems = (item) => {
  item.qty--;
  if (item.qty === 0) {
    _removeItem(item);
  }
